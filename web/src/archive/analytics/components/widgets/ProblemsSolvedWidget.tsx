...
export function ProblemsSolvedWidget({
  settings,
  size = 'normal',
  onUpdateSettings,
      ...
      case '6months': {
        ...
        for (let i = 0; i < last180Days.length; i += 7) {
          ...
          const avgProblems = Math.round(
            weekChunk.reduce((sum, day) => sum + day.problems, 0) /
              weekChunk.length,
          );
      ...
      >
        ...
        <Select
          ...
          onValueChange={(value: TimeRangeOption) =>
            onUpdateSettings?.({ ...settings, timeRange: value })
          }
        >
                ...
                content={props => {
                  ...
                  if (timeRange === '6months' && data.originalData) {
                    ...
                    const weekEnd = format(
                      parseISO(
                        data.originalData[data.originalData.length - 1]?.date ||
                          data.date,
                      ),
                      'MMM d, yyyy',
                    );

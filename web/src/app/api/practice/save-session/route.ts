...
export async function POST(request: Request) {
  ...
  try {
    ...
    const studySession = await prisma.studySession.create({
      data: {
        ...
        startTime: new Date(
          Date.now() -
            answers.reduce(
              (sum: number, a: AnswerData) => sum + a.timeSpent,
              0,
            ) *
              1000,
        ),
        endTime: new Date(),
        minutesSpent: Math.ceil(
          answers.reduce((sum: number, a: AnswerData) => sum + a.timeSpent, 0) /
            60,
        ),
    ...
    const attemptPromises = answers.map(
      (answer: AnswerData, _index: number) => {

...
export async function GET() {
  ...
  try {
    ...
    const attempts = await prisma.problemAttempt.findMany({
      ...
      select: { createdAt: true, isCorrect: true, problem: { select: { exam: true, syllabusCategory: true, questionNumber: true } }, timeSpent: true }
    })
    ...
    const accuracyHistory = Object.entries(monthStats).map(([month, stats]) => ({
      month: month.split(' ')[0],
      value: stats.attempts ? Math.round((stats.correct / stats.attempts) * 100) : 0
    })).reverse()

    const volumeHistory = Object.entries(monthStats).map(([month, stats]) => ({
      month: month.split(' ')[0],
    ...
    return NextResponse.json({ accuracyHistory, volumeHistory, recentProblems })
  } catch (error) {
    console.error('Error fetching history analytics:', error)
    return NextResponse.json({ error: 'Failed to fetch history analytics' }, { status: 500 })
  }
}

...
import { format, subDays, differenceInCalendarDays, isSameMonth } from 'date-fns'
...
export async function GET() {
  ...
  try {
    ...
    const sessions = await prisma.studySession.findMany({ where: { profileId: profile.id } })
    ...
    for (const sessionData of sessions) {
      const day = format(sessionData.startTime, 'EEE') as keyof typeof weekdayStats
    ...
    const monthDays = uniqueDates.filter(d => isSameMonth(new Date(d), new Date())).length

    const weekdayDistribution = Object.entries(weekdayStats).map(([name, hours]) => ({
      name,
    ...
    for (const attempt of attempts) {
      const bucket = getBucket(attempt.studySession?.startTime ?? attempt.createdAt)
      const stat = timeStats[bucket]
      stat.attempts += 1
      if (attempt.isCorrect) stat.correct += 1
    }

    const lastWeekAccuracy = lastWeekProblems ? Math.round((lastWeekCorrect / lastWeekProblems) * 100) : 0
    const prevWeekAccuracy = prevWeekProblems ? Math.round((prevWeekCorrect / prevWeekProblems) * 100) : 0
    ...
    return NextResponse.json({
      ...
      weeklyReport: {
        ...
        studyHours: { current: Math.round(lastWeekMinutes / 60 * 10) / 10, previous: Math.round(prevWeekMinutes / 60 * 10) / 10 },

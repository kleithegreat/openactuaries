...
export default function SessionSummary({ problems: _problems, answers, onNewSession }: SessionSummaryProps) {
  const correctAnswers = answers.filter(a => a.isCorrect)
  const accuracy = answers.length > 0 ? (correctAnswers.length / answers.length) * 100 : 0
  
  const avgTime = answers.length > 0 
    ? answers.reduce((sum: number, answer: Answer) => sum + answer.timeSpent, 0) / answers.length
    : 0
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <div className="text-2xl font-semibold text-foreground">{accuracy.toFixed(0)}%</div>
            ...
            <div className="text-center">
              ...
              <div className="text-2xl font-semibold text-foreground">{Math.round(avgTime)}s</div>
              <div className="text-foreground-secondary">Avg Time per Problem</div>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-2xl font-semibold text-foreground">{correctAnswers.length} / {answers.length}</div>

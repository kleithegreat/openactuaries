...
): asserts user is User & { profile: UserProfile } {
  if (!user.profile) {
...
async function main() {
  ...
  if (problems.length === 0) {
    throw new Error(
      'No problems found in database. Please run the problem seed script first.',
    );
  }
...
async function createUsers(): Promise<UserWithProfile[]> {
  ...
  const userData = [
    testUser,
    ...(await Promise.all(
      Array.from({ length: NUM_USERS - 1 }, async () => ({
...
async function createExamRegistrations(users: UserWithProfile[]) {
  ...
  const examRegistrations = users.flatMap(user => {
    ...
    const exams = registerForBoth
      ? ['P', 'FM']
      : [faker.helpers.arrayElement(['P', 'FM'])];

    return exams.map(examType => {
      const examWindow = faker.helpers.arrayElement(
        EXAM_WINDOWS[examType as keyof typeof EXAM_WINDOWS],
      );
  ...
  for (let i = 0; i < users.length; i += CHUNK_SIZE) {
    ...
    for (const user of userChunk) {
      ...
      for (let j = 0; j < STUDY_SESSIONS_PER_USER; j++) {
        ...
        const sessionProblems = faker.helpers.arrayElements(
          problems,
    ...
    console.log(
      `Processed ${Math.min(i + CHUNK_SIZE, users.length)}/${users.length} users`,
    );
  }
}

...
describe('StartPracticingButton', () => {
  ...
  it('redirects existing users to home', async () => {
    global.fetch = jest.fn(() =>
      Promise.resolve({ json: () => Promise.resolve({ goalType: 'PROBLEMS', goalAmount: 5, examRegistrations: [1] }) })
    ) as jest.Mock
    const push = jest.fn()
    jest.spyOn(require('next/navigation'), 'useRouter').mockReturnValue({ push })
    ...
    await waitFor(() => expect(screen.getByRole('button')).toHaveTextContent('Go to Dashboard'))
    fireEvent.click(screen.getByRole('button'))
